...
CREATE UNIQUE INDEX IF NOT EXISTS "rider_app_password_credentials_normalizedUsername_key"
  ON "rider_app_password_credentials"("normalizedUsername");
CREATE UNIQUE INDEX IF NOT EXISTS "rider_app_password_credentials_userId_key"
  ON "rider_app_password_credentials"("userId");

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'rider_app_password_credentials_userId_fkey') THEN
