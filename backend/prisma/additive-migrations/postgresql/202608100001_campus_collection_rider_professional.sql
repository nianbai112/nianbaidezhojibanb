ALTER TABLE "campus_map_collection_tasks"
  ADD COLUMN IF NOT EXISTS "allowedClients" JSONB NOT NULL DEFAULT '["miniapp"]',
  ADD COLUMN IF NOT EXISTS "objectTypes" JSONB NOT NULL DEFAULT '["road","building","entrance","facility","issue"]',
...
ALTER TABLE "campus_map_collection_attachments" ADD COLUMN IF NOT EXISTS "objectId" TEXT;
CREATE UNIQUE INDEX IF NOT EXISTS "campus_map_collection_objects_sessionId_clientObjectId_key" ON "campus_map_collection_objects"("sessionId", "clientObjectId");
CREATE INDEX IF NOT EXISTS "campus_map_collection_objects_sessionId_objectType_recordedAt_idx" ON "campus_map_collection_objects"("sessionId", "objectType", "recordedAt");
CREATE INDEX IF NOT EXISTS "campus_map_collection_objects_reviewStatus_updatedAt_idx" ON "campus_map_collection_objects"("reviewStatus", "updatedAt");
CREATE INDEX IF NOT EXISTS "campus_map_collection_attachments_objectId_uploadedAt_idx" ON "campus_map_collection_attachments"("objectId", "uploadedAt");
