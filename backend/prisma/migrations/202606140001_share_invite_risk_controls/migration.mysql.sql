ALTER TABLE `share_settings`
  ...
  ADD COLUMN IF NOT EXISTS `rewardReleaseMode` VARCHAR(32) NOT NULL DEFAULT 'immediate',
  ADD COLUMN IF NOT EXISTS `rewardDelayHours` INTEGER NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS `inviterWhitelist` JSON,
  ADD COLUMN IF NOT EXISTS `inviterBlacklist` JSON,
  ADD COLUMN IF NOT EXISTS `inviteeBlacklist` JSON,
  ADD COLUMN IF NOT EXISTS `blockedPhonePrefixes` JSON;

ALTER TABLE `share_invites`
  ADD COLUMN IF NOT EXISTS `ip` TEXT,
  ADD COLUMN IF NOT EXISTS `deviceId` TEXT,
  ADD COLUMN IF NOT EXISTS `userAgent` TEXT,
  ADD COLUMN IF NOT EXISTS `riskReasons` JSON,
  ADD COLUMN IF NOT EXISTS `rewardReleaseMode` VARCHAR(32),
  ADD COLUMN IF NOT EXISTS `rewardReadyAt` DATETIME(3);

CREATE INDEX IF NOT EXISTS `share_invites_region_inviter_created_idx` ON `share_invites`(`regionId`, `inviterId`, `createdAt`);
CREATE INDEX IF NOT EXISTS `share_invites_region_ip_created_idx` ON `share_invites`(`regionId`, `ip`(191), `createdAt`);
CREATE INDEX IF NOT EXISTS `share_invites_region_device_created_idx` ON `share_invites`(`regionId`, `deviceId`(191), `createdAt`);
