...
import { createThrottlerDefinitions } from "./config/throttling.config";
  ...
  imports: [
    ...
    ThrottlerModule.forRootAsync({
      ...
      useFactory: (redisService: RedisService) => ({
        ...
        throttlers: createThrottlerDefinitions(),
      }),
    }),
    // Scheduled jobs run exclusively in the standalone Worker process.
    // Keeping ScheduleModule out of the HTTP app prevents duplicate Cron/Interval work.
    PrismaModule,
    RedisModule,
    LoggerModule,
    UploadModule,
