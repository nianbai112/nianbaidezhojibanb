  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  private loadAllowedOrigins() {
    ...
    const socketOrigins =
      this.config.get("SOCKET_ALLOWED_ORIGINS") || corsOrigin;
  ...
  async handleConnection(client: Socket) {
    try {
      ...
      if (!this.validateOrigin(client)) {
        ...
        await this.logSecurity(client, "connection_rejected", "非法来源", {
          origin,
        });
        client.disconnect();
        return;
      ...
      if (!client.data.isAdmin) {
        await this.userAccess.assertActiveUser(
          client.data.userId,
          "连接实时服务",
        );
      }
    ...
    try {
      ...
      if (this.isLegacyMessageWriteDisabled()) {
        throw new WsException(
          "旧版 Socket.IO 消息发送已停用，请使用原生实时通道",
        );
      }
      ...
      const maxMsg = client.data.isAdmin
      ...
      if (await this.detectSensitiveWords(data.content)) {
        this.logger.warn(
          `敏感词检测触发: userId=${userId} conv=${data.conversationId}`,
        );
        ...
        select: {
          ...
          user: {
            select: {
              ...
              userType: true,
              settings: {
                select: { messagePermission: true, allowMessage: true },
              },
            },
          },
        },
      ...
      for (const om of otherMembers) {
        ...
        if (member.conversation.type === "private") {
          ...
          if (!permission.allowed) {
            throw new WsException(
              permission.message || "对方当前不允许接收你的私信",
            );
          }
        }
      }
  ...
  async handleRecallMessage(
    ...
    try {
      ...
      if (msg.senderId !== userId)
        throw new WsException("只能撤回自己发送的消息");
  ...
  async handleAdminRecallMessage(
    @MessageBody()
    data: { messageId: string; conversationId: string; reason?: string },
    @ConnectedSocket() client: Socket,
  ) {
    try {
      this.assertAdmin(client);
  ...
  async handleAdminToggleConversation(
    ...
    try {
      ...
      this.server
        ...
        .emit(data.blocked ? "conversationBlocked" : "conversationUnblocked", {
          conversationId: data.conversationId,
        });
  ...
  private assertAdmin(client: Socket) {
    if (!client.data.isAdmin) {
      this.logger.warn(
        `非管理员尝试调用管理员事件: userId=${client.data.userId}`,
      );
