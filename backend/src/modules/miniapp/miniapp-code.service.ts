...
export class MiniappCodeService {
  ...
  /** 合法页面类型（与 admin layoutSchemas、page-renderer 三方协议一致） */
  private assertPageType(pageType: string): string {
    const t = String(pageType || '');
    if (!/^[a-z][a-z0-9-]{0,31}$/.test(t)) {
      throw new BadRequestException(`非法的页面类型：${t}`);
    }
    return t;
  }
  ...
  private layoutDir(): string {
    const dir = this.assertSourceDir();
    const layoutDir = path.join(dir, 'config', 'layout');
    fs.mkdirSync(layoutDir, { recursive: true });
    return layoutDir;
  }

  getPageLayout(pageType: string) {
    const t = this.assertPageType(pageType);
    const file = path.join(this.layoutDir(), `${t}.json`);
    if (!fs.existsSync(file)) {
      return { success: true, data: { pageType: t, bundled: false, layout: null } };
    }
    const layout = JSON.parse(fs.readFileSync(file, 'utf8'));
    return { success: true, data: { pageType: t, bundled: true, layout } };
  }

  savePageLayout(pageType: string, layout: any) {
    const t = this.assertPageType(pageType);
    if (!layout || !Array.isArray(layout.components)) {
      throw new BadRequestException('布局格式无效：缺少 components 数组');
    }
    if (layout.components.length > 60) {
      throw new BadRequestException('组件数量过多（上限 60）');
    }
    const file = path.join(this.layoutDir(), `${t}.json`);
    const payload = {
    ...
    if (fs.existsSync(file)) this.backupFile(file);
    fs.writeFileSync(file, JSON.stringify(payload, null, 2) + '\n', 'utf8');
    this.rebuildBundledLayouts();
    return { success: true, message: `已写入代码包：config/layout/${t}.json`, data: { pageType: t, count: payload.components.length } };
  }

  removePageLayout(pageType: string) {
    const t = this.assertPageType(pageType);
    const file = path.join(this.layoutDir(), `${t}.json`);
    if (fs.existsSync(file)) {
      this.backupFile(file);
    ...
    this.rebuildBundledLayouts();
    return { success: true, message: `已移除内置布局：${t}（小程序回退为远程配置）` };
  }

  /** 扫描 config/layout/*.json 重新生成 bundled.js（page-renderer 启动时 require） */
  private rebuildBundledLayouts() {
    const dir = this.layoutDir();
    const map: Record<string, any> = {};
    for (const f of fs.readdirSync(dir)) {
      if (!/^[a-z][a-z0-9-]{0,31}\.json$/.test(f)) continue;
      try {
        map[f.replace(/\.json$/, '')] = JSON.parse(fs.readFileSync(path.join(dir, f), 'utf8'));
      } catch { /* 单个文件损坏不阻塞整体生成 */ }
    }
    const content =
      '"use strict";\n' +
      '/**\n * 内置页面布局配置（由后台「UI 编辑器 → 自由布局 → 保存到代码包」自动生成，请勿手改）。\n' +
      ' * page-renderer 优先使用本文件的配置；没有内置配置时回退到远程接口拉取。\n */\n' +
      `module.exports = ${JSON.stringify(map)};\n`;
    fs.writeFileSync(path.join(dir, 'bundled.js'), content, 'utf8');
  }
