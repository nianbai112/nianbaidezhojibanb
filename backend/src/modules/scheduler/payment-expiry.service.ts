...
export class PaymentExpiryService {
  ...
  constructor(
    ...
    private readonly redis: RedisService = { withLock: async (_key: string, _ttl: number, task: () => Promise<any>) => task() } as unknown as RedisService,
  ) {}
  ...
  async runScheduledReconciliation() {
    ...
    this.running = true;
    try {
      await this.redis.withLock('scheduler:payment-expiry', 55, () => this.reconcileAndRelease());
    } finally {
      this.running = false;
    }
  }
  ...
  async reconcileAndRelease() {
    const terminalPayments = await this.paymentService.reconcileExpiredPayments();
    ...
    for (const job of jobs) {
      ...
      try {
        ...
        await this.prisma.paymentReservationRelease.update({
          where: { id: job.id },
          data: { status: 'failed', lastError: String(error?.message || 'unknown').slice(0, 500) },
        });
        this.logger.error(
  ...
  private async expireOrphanShopOrders() {
    const staleOrders = await this.prisma.order.findMany({
      where: { status: 'PENDING_PAY', createdAt: { lte: new Date(Date.now() - 15 * 60 * 1000) } },
      select: { id: true },
      take: 100,
    });
    ...
    const activePayments = await this.prisma.paymentOrder.findMany({
      where: { bizType: 'order', bizId: { in: staleOrders.map((order) => order.id) }, status: { in: ['pending', 'paying'] } },
      select: { bizId: true },
    });
    const activeOrderIds = new Set(activePayments.map((payment) => payment.bizId));
    for (const order of staleOrders) {
      if (!activeOrderIds.has(order.id)) await this.shopService.expirePendingPayment(order.id);
    }
  }

  private resolveReleaseHandler(bizType: string): ((bizId: string) => Promise<any>) | null {
    switch (bizType) {
      case 'mall_order':
