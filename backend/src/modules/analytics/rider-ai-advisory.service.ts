...
export class RiderAiAdvisoryService {
  ...
  private async readConfigRaw(): Promise<RiderAiConfig> {
    const row = await this.prisma.config.findUnique({ where: { key: this.configKey } }).catch(() => null);
  ...
  async shouldRunScheduledAnalysis(now = new Date()) {
    ...
    if (!config.enabled || config.analysisInterval === 'manual') {
      return { due: false, reason: 'disabled_or_manual', config: this.maskRiderAiConfig(config) };
    }
  ...
  async saveConfig(data: any = {}) {
    ...
    const next: RiderAiConfig = {
      ...
      apiBaseUrl: String(data.apiBaseUrl || data.api_base_url || current.apiBaseUrl || ''),
      apiKey: incomingKey && !/^\*+$/.test(incomingKey) ? incomingKey : current.apiKey,
      model: String(data.model || current.model || 'deepseek-chat'),
      temperature: this.clampNumber(data.temperature, 0, 2, current.temperature),
      maxTokens: this.clampNumber(data.maxTokens ?? data.max_tokens, 100, 32000, current.maxTokens),
      dailyCallLimit: this.clampNumber(data.dailyCallLimit ?? data.daily_call_limit, 0, 10000, current.dailyCallLimit),
      dailyCostLimit: this.clampNumber(data.dailyCostLimit ?? data.daily_cost_limit, 0, 100000, current.dailyCostLimit),
      analysisInterval: this.normalizeInterval(data.analysisInterval || data.analysis_interval || current.analysisInterval),
      analysisScope: this.normalizeScope(data.analysisScope || data.analysis_scope || current.analysisScope),
      regionScope: String(data.regionScope || data.region_scope || current.regionScope || 'all'),
      lastRunAt: current.lastRunAt || null,
      nextRunAt: this.nextRunAt(this.normalizeInterval(data.analysisInterval || data.analysis_interval || current.analysisInterval)),
      lastStatus: current.lastStatus || 'not_run',
    };
    await this.prisma.config.upsert({
      where: { key: this.configKey },
      update: { value: next, group: 'analytics' },
      create: { key: this.configKey, value: next, group: 'analytics', desc: '骑手算法 AI 建议配置' },
    });
  ...
  async updateSuggestionStatus(id: string, dto: any = {}) {
    const value = await this.readList(this.suggestionsKey);
    const nextStatus = ['pending', 'accepted', 'applied', 'dismissed'].includes(String(dto.status)) ? String(dto.status) : 'pending';
    const next = value.map((item: any) => item.id === id ? {
      ...item,
      status: nextStatus,
      operator_note: dto.operator_note || dto.operatorNote || item.operator_note || '',
      updated_at: new Date().toISOString(),
    } : item);
    await this.writeList(this.suggestionsKey, next, '骑手算法 AI 建议');
    return { success: true, data: next.find((item: any) => item.id === id) || null };
  }
  ...
  async runAnalysis(metrics: any = {}, triggerType = 'manual') {
    ...
    let modelUsage: any = {};
    let modelError = '';
    let status = config.enabled ? 'completed_builtin_missing_model_config' : 'completed_builtin_advisory';
    if (this.canCallConfiguredModel(config)) {
      try {
        ...
        if (modelResult.suggestions.length) {
          suggestions = [...modelResult.suggestions, ...builtInSuggestions].slice(0, 30);
    ...
    const log = {
      ...
      input_tokens: Number(modelUsage.prompt_tokens || modelUsage.input_tokens || 0),
      output_tokens: Number(modelUsage.completion_tokens || modelUsage.output_tokens || 0),
    ...
    await this.writeList(this.runLogsKey, [...(await this.readList(this.runLogsKey)), log].slice(-200), '骑手算法 AI 分析日志');
    await this.prisma.config.upsert({
      where: { key: this.configKey },
      update: { value: { ...config, lastRunAt: startedAt.toISOString(), nextRunAt: this.nextRunAt(config.analysisInterval), lastStatus: log.status }, group: 'analytics' },
      create: { key: this.configKey, value: { ...config, lastRunAt: startedAt.toISOString(), nextRunAt: this.nextRunAt(config.analysisInterval), lastStatus: log.status }, group: 'analytics', desc: '骑手算法 AI 建议配置' },
    });
    return { success: true, data: { log, suggestions } };
  }

  private canCallConfiguredModel(config: RiderAiConfig) {
    return config.enabled === true && !!String(config.apiKey || '').trim() && !!String(config.apiBaseUrl || '').trim();
  }
  ...
  private async callConfiguredModel(metrics: any, config: RiderAiConfig) {
    ...
    if (!response.ok) {
      throw new Error(body?.error?.message || body?.message || `模型请求失败：${response.status}`);
    }
  ...
  private normalizeModelSuggestions(raw: any, config: RiderAiConfig) {
    ...
    return list.slice(0, 10)
      .map((item: any, index: number) => ({
        ...
        status: 'pending',
        suggestion_type: String(item?.suggestion_type || item?.type || 'algorithm'),
        target_algorithm: ['errand', 'takeaway', 'all'].includes(String(item?.target_algorithm || item?.targetAlgorithm))
          ? String(item?.target_algorithm || item?.targetAlgorithm)
          : config.analysisScope === 'takeaway'
            ? 'takeaway'
            : 'errand',
        title: String(item?.title || '').trim().slice(0, 80),
        suggested_change: String(item?.suggested_change || item?.suggestedChange || item?.change || '').trim().slice(0, 500),
        current_metric_evidence: String(item?.current_metric_evidence || item?.evidence || '').trim().slice(0, 300),
        expected_impact: String(item?.expected_impact || item?.impact || '').trim().slice(0, 300),
        risk_warning: String(item?.risk_warning || item?.warning || '模型建议，不自动生效，需要人工确认').trim().slice(0, 300),
        confidence: ['low', 'medium', 'high'].includes(String(item?.confidence)) ? String(item.confidence) : 'medium',
  ...
  private createBuiltInSuggestions(metrics: any = {}, config: RiderAiConfig) {
    ...
    const fulfillment = metrics.fulfillment_metrics?.overall || summary.fulfillment || {};
    const suggestions: any[] = [];
    const now = new Date().toISOString();
    const push = (item: any) => suggestions.push({
    ...
    if (topTag === 'cake' || summary.high_risk > 0) {
      push({
        ...
        title: '收紧高风险跑腿任务派单',
        suggested_change: '蛋糕、液体、贵重物订单默认禁止叠单，并缩小到同片区或附近片区推送。',
    ...
    if ((summary.evidence_required || 0) > 0) {
      push({
        ...
        target_algorithm: 'errand',
        title: '检查高风险订单拍照闭环',
        suggested_change: '对需要证据的订单核查取件照和送达照是否完整，缺失时提醒骑手补充。',
        evidence: `需证据样本 ${summary.evidence_required} 条`,
        impact: '提高售后纠纷处理依据',
      });
    }
    const totalOrders = Number(fulfillment.total_orders || 0);
    if (totalOrders >= 10 && Number(fulfillment.acceptance_rate || 0) > 0 && Number(fulfillment.acceptance_rate) < 80) {
      push({
        suggestion_type: 'dispatch',
        target_algorithm: 'errand',
        title: '接单成功率偏低，建议调整推送和价格策略',
        suggested_change: '扩大低风险订单的首轮推送范围，检查基础费和认证骑手加价是否导致骑手不愿接单。',
        evidence: `近周期接单成功率 ${fulfillment.acceptance_rate}%，样本 ${totalOrders} 单`,
        impact: '提升待接单转接单效率，减少用户下单后长时间等待',
        confidence: Number(fulfillment.acceptance_rate) < 65 ? 'high' : 'medium',
      });
    }
    if (totalOrders >= 10 && Number(fulfillment.timeout_rate || 0) > 10) {
      push({
        ...
        suggested_change: '按任务类型、楼栋区域和高峰时段增加 ETA 缓冲；高风险物品禁止叠单或缩小同片区推送。',
    ...
    if (totalOrders >= 10 && Number(fulfillment.cancel_rate || 0) > 10) {
      push({
        ...
        suggested_change: '拆分未接单取消、骑手接单后取消、用户主动取消三类原因，并对无人接单订单提前触发兜底推送。',
    ...
    if (totalOrders >= 10 && Number(fulfillment.incident_rate || 0) > 1) {
      push({
        ...
        suggested_change: '蛋糕、贵重、大件、液体类任务要求取件照和送达照；事故高发时段降低骑手叠单上限。',
  ...
  private async readList(key: string) {
    const row = await this.prisma.config.findUnique({ where: { key } }).catch(() => null);
  ...
  private async writeList(key: string, list: any[], desc: string) {
    await this.prisma.config.upsert({
      ...
      create: { key, value: { list, updated_at: new Date().toISOString() }, group: 'analytics', desc },
    });
  }
  ...
  private normalizeInterval(value: any): RiderAiConfig['analysisInterval'] {
    return ['manual', 'hourly', 'six_hours', 'daily', 'weekly'].includes(String(value)) ? String(value) as any : 'manual';
  }

  private normalizeScope(value: any): RiderAiConfig['analysisScope'] {
    return ['all', 'errand', 'takeaway'].includes(String(value)) ? String(value) as any : 'all';
  }

  private nextRunAt(interval: RiderAiConfig['analysisInterval']) {
    if (interval === 'manual') return null;
    const minutes = interval === 'hourly' ? 60 : interval === 'six_hours' ? 360 : interval === 'daily' ? 1440 : 10080;
