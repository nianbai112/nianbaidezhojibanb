...
describe('AuditService AI review content closure', () => {
  const createAiReviewService = (decision: 'approve' | 'reject' | 'manual') => {
    const prisma = {
      post: { findUnique: jest.fn().mockResolvedValue({ id: 'post-1', userId: 'user-1', regionId: 'region-1', title: 'title', content: 'content' }) },
    ...
    return { prisma, service: new AuditService(prisma as any, { delPattern: jest.fn() } as any, aiRuntime as any, {} as any) };
  };
