...
export class CampusMapCollectionController {
  ...
  async listTasks(
    ...
    return this.service.listTasks(regionId, {
      status,
      page: Number(page) || 1,
  ...
  @Get(
    "admin/campus-map/collections/:regionId/tasks/:taskId/sessions/:sessionId",
  )
  @UseGuards(JwtGuard, AdminGuard, AdminPermissionGuard)
  @RequirePermission("region:view")
  @ApiBearerAuth()
  ...
  async reviewCollectionObject(
    ...
    const adminId = await this.assertAdminRegion(req, regionId);
    return this.service.reviewCollectionObject(
      regionId,
      objectId,
      dto,
      adminId,
  ...
  @Get("admin/campus-map/collections/:regionId/objects")
  @UseGuards(JwtGuard, AdminGuard, AdminPermissionGuard)
  @RequirePermission("region:view")
  @ApiBearerAuth()
  async listObjects(
    ...
    await this.assertAdminRegion(req, regionId);
    return this.service.listObjects(regionId, {
      reviewStatus,
      taskId,
      objectType,
      page: Number(page) || 1,
  ...
  async rotateAccessCode(
    ...
    const adminId = await this.assertAdminRegion(req, regionId);
    const access = await this.service.rotateAccessCode(
      regionId,
      taskId,
      adminId,
    );
  ...
  resolveCollectorContext(
    ...
    return this.service.resolveCollectorContext(
      code,
      this.collectorUserId(req),
    );
  }
  ...
  startRiderSession(
    ...
    return this.service.startRiderSession(
      taskId,
      this.collectorUserId(req),
      dto,
    );
  ...
  uploadPointBatch(
    ...
    return this.service.uploadPointBatch(
      sessionId,
      this.collectorUserId(req),
      Number(batchNo),
      dto,
  ...
  createCollectionObject(
    ...
    return this.service.createCollectionObject(
      sessionId,
      this.collectorUserId(req),
      dto,
    );
  ...
  private collectorUserId(req: AuthenticatedRequest) {
    if (!req.user?.sub || req.user.isAdmin)
      throw new ForbiddenException("采集端不接受管理员登录态");
    return req.user.sub;
  }
}
