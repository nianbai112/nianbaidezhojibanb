...
const LEGACY_SETUP_LOCK_PATH = path.resolve(
  process.cwd(),
  "storage",
  "setup.lock",
);
...
function isStrongPassword(password: string): {
  ...
  if (password.length < 12)
    return { valid: false, reason: "密码长度至少 12 位" };
  if (password.length > 128)
    return { valid: false, reason: "密码长度不可超过 128 位" };
  if (countCharClasses(password) < 3)
    return {
      valid: false,
      reason: "密码必须包含大写字母、小写字母、数字、特殊字符中至少 3 种",
    };
  for (const p of WEAK_PASSWORD_PATTERNS) {
    if (p.test(password))
      return { valid: false, reason: "密码包含常见弱词，请使用更强密码" };
  }
  return { valid: true };
}
...
function validateJwtSecret(secret: string | undefined): {
  ...
  if (!secret || !secret.trim())
    return { valid: false, reason: "JWT_SECRET 未配置" };
  const s = secret.trim();
  if (s.length < 32)
    return { valid: false, reason: "JWT_SECRET 长度至少 32 位" };
  if (s.length > 512)
    return { valid: false, reason: "JWT_SECRET 长度不可超过 512 位" };
  if (
    /^(change[-_]?me|your[-_].*secret|super[-_]secret|default[-_].*secret|example[-_].*secret|test[-_].*secret)/i.test(
      s,
    )
  )
    return {
      valid: false,
      reason: "JWT_SECRET 不能使用占位值，请随机生成强密钥",
    };
  return { valid: true };
}
...
export class SetupService {
  ...
  private isSetupWizardMode(): boolean {
    const installed = String(
      this.configService.get("DB_IS_INSTALLED") ??
        process.env.DB_IS_INSTALLED ??
        "",
    ).toLowerCase();
    const wizard = String(
  ...
  async isInitialized(): Promise<boolean> {
    ...
    for (const lockPath of [
      ...new Set([SETUP_LOCK_PATH, LEGACY_SETUP_LOCK_PATH]),
    ]) {
      try {
  ...
  async checkEnvironment(token: string, dto: Partial<SetupInitDto> = {}) {
    ...
    const systemChecks = [
      ...
      { name: "PM2", script: "pm2 -v", required: true },
      {
        ...
        script:
          "mysql --version || mariadb --version || psql --version || postgres --version || postmaster --version",
        required: true,
      },
      {
        name: "Redis",
    ...
    if (!dbUrl) {
      add(
        "DATABASE_URL",
        "failed",
        "未填写 MySQL 或 PostgreSQL 数据库连接信息",
      );
    ...
    if (dbUrl && isSupportedDatabaseUrl(dbUrl)) {
      try {
        ...
        if (schemaChanged) {
          const generateResult = await runShellCommand(
            "npx prisma generate",
            120000,
          );
    ...
    add("微信支付", "passed", "首次安装不检查；开通支付后在后台支付配置里补齐");
    ...
    if (hasSuperAdmin) {
      add(
        ...
        isWeakPassword ? "超级管理员密码为弱口令，请立即修改" : "密码强度合格",
      );
    }
  ...
  private async writeEnvUpdates(
    ...
    await fs.promises.writeFile(ENV_PATH, updateContent(content));
    ...
    if (BACKEND_ENV_PATH !== ENV_PATH) {
      try {
        ...
        await fs.promises.mkdir(path.dirname(BACKEND_ENV_PATH), {
          recursive: true,
        });
        let backendContent = "";
        try {
  ...
  private toDatabasePrismaSchema(
    raw: string,
  ...
  private scheduleRestartAfterSetup() {
    ...
    if (!managedByPm2) {
      this.logger.warn(
        "初始化完成后需要手动重启 API、Worker、Realtime，当前进程未检测到 PM2 管理",
      );
      return false;
    }

    const safePm2Name = (value: string | undefined, fallback: string) => {
      const name = String(value || "").trim();
      return /^[a-zA-Z0-9_.-]+$/.test(name) ? name : fallback;
    };
    // API 放在最后重启，确保 PM2 客户端先向守护进程提交完整的三服务重载指令。
    const serviceNames = [
      safePm2Name(
        process.env.PM2_WORKER_NAME || process.env.WORKER_PM2_NAME,
        "lingmeng-worker",
      ),
      safePm2Name(
        process.env.PM2_REALTIME_NAME || process.env.REALTIME_PM2_NAME,
        "lingmeng-realtime",
      ),
      safePm2Name(process.env.PM2_NAME, "lingmeng-backend"),
    ];

    this.logger.log(
      `初始化完成，准备重载 PM2 服务以重新加载 .env: ${serviceNames.join(", ")}`,
    );
    setTimeout(() => {
      this.logger.log("执行安装后三服务重载");
      const pm2 = this.spawnDetachedPm2([
        "restart",
        ...serviceNames,
        "--update-env",
      ]);
      pm2.once("error", (error) => {
        this.logger.error(`PM2 三服务重载失败，请手动重启: ${error.message}`);
      });
      pm2.unref();
    }, 2000).unref();
    return true;
  ...
  private spawnDetachedPm2(args: string[]) {
    return childProcess.spawn("pm2", args, {
      detached: true,
      stdio: "ignore",
      env: process.env,
    });
  ...
  async init(dto: SetupInitDto, token: string) {
    ...
    if (!passCheck.valid)
      throw new BadRequestException(passCheck.reason || "密码不符合要求");
    ...
    try {
      ...
      if (!migration.ready) {
        const command =
          "node scripts/migrate-release.cjs install && npm run db:generate";
      ...
      await prisma.config.upsert({
        ...
        update: {
          value: {
            ...
            wxMiniAppid: maskSecret(
              this.effectiveConfig("WX_MINI_APPID", envUpdates),
            ),
  ...
  private async seedBaseData(prisma: PrismaClient | PrismaService) {
    ...
    const basePermissions = [
      ...
      {
        code: "merchant:collection",
        name: "管理商品采集",
        module: "merchant",
        action: "collection",
      },
      {
        code: "merchant:config",
        name: "管理商家配置",
        module: "merchant",
        action: "config",
      },
      ...
      {
        code: "errand:config:view",
        name: "查看跑腿配置",
        module: "errand",
        action: "config:view",
      },
      {
        code: "errand:config:update",
        name: "更新跑腿配置",
        module: "errand",
        action: "config:update",
      },
      {
        code: "errand:item-size:view",
        name: "查看物品大小",
        module: "errand",
        action: "item-size:view",
      },
      {
        code: "errand:item-size:create",
        name: "创建物品大小",
        module: "errand",
        action: "item-size:create",
      },
      {
        code: "errand:item-size:update",
        name: "更新物品大小",
        module: "errand",
        action: "item-size:update",
      },
      {
        code: "errand:item-size:delete",
        name: "删除物品大小",
        module: "errand",
        action: "item-size:delete",
      },
      {
        code: "errand:pickup-point:view",
        name: "查看取件点",
        module: "errand",
        action: "pickup-point:view",
      },
      {
        code: "errand:pickup-point:create",
        name: "创建取件点",
        module: "errand",
        action: "pickup-point:create",
      },
      {
        code: "errand:pickup-point:update",
        name: "更新取件点",
        module: "errand",
        action: "pickup-point:update",
      },
      {
        code: "errand:pickup-point:delete",
        name: "删除取件点",
        module: "errand",
        action: "pickup-point:delete",
      },
      {
        code: "errand:stats:view",
        name: "查看跑腿统计",
        module: "errand",
        action: "stats:view",
      },
      ...
      {
        code: "coupon:redeem-code:view",
        name: "查看卡券兑换码",
        module: "coupon",
        action: "redeem-code:view",
      },
      {
        code: "coupon:redeem-code:create",
        name: "生成卡券兑换码",
        module: "coupon",
        action: "redeem-code:create",
      },
      {
        code: "coupon:redeem-code:edit",
        name: "管理卡券兑换码",
        module: "coupon",
      ...
      // 二手交易
      {
        code: "secondhand:view",
        name: "查看二手交易",
        module: "secondhand",
        action: "view",
      },
      {
        code: "secondhand:audit",
        name: "审核二手交易",
        module: "secondhand",
        action: "audit",
      },
      {
        code: "secondhand:config",
        name: "二手配置",
        module: "secondhand",
        action: "config",
      },
      // 打卡管理
      {
        code: "punchIn:location:list",
        name: "查看打卡点列表",
        module: "punchIn",
        action: "location:list",
      },
      {
        code: "punchIn:location:create",
        name: "创建打卡点",
        module: "punchIn",
        action: "location:create",
      },
      {
        code: "punchIn:location:update",
        name: "更新打卡点",
        module: "punchIn",
        action: "location:update",
      },
      {
        code: "punchIn:location:delete",
        name: "删除打卡点",
        module: "punchIn",
        action: "location:delete",
      },
      {
        code: "punchIn:record:list",
        name: "查看打卡记录",
        module: "punchIn",
        action: "record:list",
      },
      ...
      // 笔记付费置顶
      {
        code: "topup:package:list",
        name: "查看置顶套餐",
        module: "topup",
        action: "package:list",
      },
      {
        code: "topup:package:create",
        name: "创建置顶套餐",
        module: "topup",
        action: "package:create",
      },
      {
        code: "topup:package:update",
        name: "更新置顶套餐",
        module: "topup",
        action: "package:update",
      },
      {
        code: "topup:package:delete",
        name: "删除置顶套餐",
        module: "topup",
        action: "package:delete",
      },
      {
        code: "topup:order:list",
        name: "查看置顶订单",
        module: "topup",
        action: "order:list",
      },
      ...
      {
        code: "club:member:list",
        name: "查看社团成员",
        module: "club",
        action: "member:list",
      },
      {
        code: "club:member:delete",
        name: "移除社团成员",
        module: "club",
      ...
      // 用户引导
      {
        code: "userGuidance:list",
        name: "查看引导页列表",
        module: "userGuidance",
        action: "list",
      },
      {
        code: "userGuidance:create",
        name: "创建引导页",
        module: "userGuidance",
        action: "create",
      },
      {
        code: "userGuidance:update",
        name: "更新引导页",
        module: "userGuidance",
        action: "update",
      },
      {
        code: "userGuidance:delete",
        name: "删除引导页",
        module: "userGuidance",
        action: "delete",
      },
      // 通讯录
      {
        code: "contacts:category:list",
        name: "查看通讯录分类",
        module: "contacts",
        action: "category:list",
      },
      {
        code: "contacts:category:create",
        name: "创建通讯录分类",
        module: "contacts",
        action: "category:create",
      },
      {
        code: "contacts:category:update",
        name: "更新通讯录分类",
        module: "contacts",
        action: "category:update",
      },
      {
        code: "contacts:category:delete",
        name: "删除通讯录分类",
        module: "contacts",
      ...
      // 微信文章
      {
        code: "wechatArticle:list",
        name: "查看微信文章",
        module: "wechatArticle",
        action: "list",
      },
      {
        code: "wechatArticle:delete",
        name: "删除微信文章",
        module: "wechatArticle",
        action: "delete",
      },
      ...
      {
        code: "userTitle:create",
        name: "创建头衔",
        module: "userTitle",
        action: "create",
      },
      {
        code: "userTitle:update",
        name: "更新头衔",
        module: "userTitle",
        action: "update",
      },
      {
        code: "userTitle:delete",
        name: "删除头衔",
        module: "userTitle",
        action: "delete",
      },
      {
        code: "userTitle:code:list",
        name: "查看兑换码",
        module: "userTitle",
        action: "code:list",
      },
      {
        code: "userTitle:code:create",
        name: "生成兑换码",
        module: "userTitle",
        action: "code:create",
      },
      // 贴纸管理
      {
        code: "sticker:category:list",
        name: "查看贴纸分类",
        module: "sticker",
        action: "category:list",
      },
      {
        code: "sticker:category:create",
        name: "创建贴纸分类",
        module: "sticker",
        action: "category:create",
      },
      {
        code: "sticker:category:update",
        name: "更新贴纸分类",
        module: "sticker",
        action: "category:update",
      },
      {
        code: "sticker:category:delete",
        name: "删除贴纸分类",
        module: "sticker",
        action: "category:delete",
      },
      ...
      // 爆照评选
      {
        code: "photoContest:view",
        name: "查看爆照评选",
        module: "photoContest",
        action: "view",
      },
      {
        code: "photoContest:audit",
        name: "审核爆照评选",
        module: "photoContest",
        action: "audit",
      },
      {
        code: "photoContest:config",
        name: "配置爆照评选",
        module: "photoContest",
        action: "config",
      },
      {
        code: "photoContest:list",
        name: "查看爆照评选列表",
        module: "photoContest",
        action: "list",
      },
      ...
      // 公众号解绑独立权限
      {
        code: "notification:binding:unbind",
        name: "解绑公众号",
        module: "notification",
      ...
      // 上传管理
      {
        code: "upload:admin:image",
        name: "上传管理图片",
        module: "upload",
        action: "admin:image",
      },
      {
        code: "upload:admin:video",
        name: "上传管理视频",
        module: "upload",
        action: "admin:video",
      },
      {
        code: "upload:admin:qrcode",
        name: "上传管理二维码",
        module: "upload",
        action: "admin:qrcode",
      },
      ...
      {
        code: "userguidance:view",
        name: "查看用户引导",
        module: "userguidance",
        action: "view",
      },
      {
        code: "userguidance:edit",
        name: "编辑用户引导",
        module: "userguidance",
        action: "edit",
      },
