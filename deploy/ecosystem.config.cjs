...
const parsedRealtimeInstances = Number.parseInt(
  process.env.REALTIME_INSTANCES || "1",
  10,
);
const realtimeInstances =
  Number.isInteger(parsedRealtimeInstances) && parsedRealtimeInstances > 0
    ? parsedRealtimeInstances
    : 1;
...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      exec_mode: realtimeInstances > 1 ? "cluster" : "fork",
      autorestart: true,
      max_memory_restart: "1000M",
      kill_timeout: 15000,
      listen_timeout: 15000,
