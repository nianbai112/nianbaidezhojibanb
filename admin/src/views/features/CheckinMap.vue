<template>
  <div class="page-shell">
    ...
    <el-tabs v-model="activeTab" @tab-change="handleTabChange">
      <el-tab-pane label="地点管理" name="locations">
        ...
        <el-table :data="locations" v-loading="locLoading" stripe>
          ...
          <el-table-column label="状态" width="80">
            <template #default="{ row }">
              <el-tag :type="row.status === 'PUBLISHED' ? 'success' : 'info'" size="small">{{ row.status === 'PUBLISHED' ? '启用' : '禁用' }}</el-tag>
            </template>
          </el-table-column>
        ...
        <div class="pagination-wrap">
          <el-pagination v-model:current-page="locPage" v-model:page-size="locPageSize" :total="locTotal"
            :page-sizes="[20,50,100]" layout="total, sizes, prev, pager, next" @change="loadLocations" />
        </div>
      </el-tab-pane>
      ...
      <el-tab-pane label="分类管理" name="categories">
        ...
        <el-table :data="categories" v-loading="catLoading" stripe>
          ...
          <el-table-column label="地点数" width="80">
            <template #default="{ row }">{{ row._count?.locations ?? 0 }}</template>
          </el-table-column>
      ...
      <el-tab-pane label="打卡记录" name="records">
        <div class="tab-toolbar">
          <el-input v-model="recFilters.keyword" placeholder="搜索用户" clearable style="width:180px" @keyup.enter="loadRecords" />
        ...
        <el-table :data="records" v-loading="recLoading" stripe>
          ...
          <el-table-column prop="location.name" label="打卡地点" width="150">
      ...
      <el-tab-pane label="评论管理" name="comments">
        ...
        <div class="pagination-wrap">
          <el-pagination v-model:current-page="comPage" v-model:page-size="comPageSize" :total="comTotal"
      ...
      <el-tab-pane label="区域配置" name="configs">
        <div class="tab-toolbar">
          <el-select v-model="configRegionId" placeholder="选择区域" style="width:200px" @change="loadConfig">
            <el-option v-for="r in regions" :key="r.id" :label="r.name" :value="r.id" />
          </el-select>
          <el-button type="primary" @click="saveConfig" :loading="configSaving" :disabled="!configRegionId">保存配置</el-button>
        </div>
        <el-form v-if="configRegionId" :model="configForm" label-width="120px" style="max-width:600px">
    ...
    <el-dialog v-model="showLocDialog" :title="editingLoc ? '编辑地点' : '新增地点'" width="600px" destroy-on-close>
      <el-form :model="locForm" label-width="100px">
        <el-form-item label="地点名称" required><el-input v-model="locForm.name" /></el-form-item>
        <el-form-item label="分类" required>
          <el-select v-model="locForm.categoryId" style="width:100%">
            <el-option v-for="c in categories" :key="c.id" :label="c.name" :value="c.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="地址">
          <div style="display:flex;gap:8px;width:100%">
            <el-input v-model="locForm.address" placeholder="点击选择地图位置" readonly style="flex:1" />
            <el-button type="primary" @click="openMapPicker">选择位置</el-button>
          </div>
        </el-form-item>
        <el-form-item label="经纬度">
          <el-input :model-value="locForm.latitude && locForm.longitude ? `${locForm.latitude}, ${locForm.longitude}` : ''" readonly placeholder="地图选择后自动填入" />
        </el-form-item>
        <el-form-item label="描述"><el-input v-model="locForm.description" type="textarea" :rows="2" /></el-form-item>
    ...
    <el-dialog v-model="showMapPicker" title="选择位置" width="700px" destroy-on-close>
      <div style="height:400px;background:#f0f0f0;display:flex;align-items:center;justify-content:center;border-radius:8px">
        <p style="color:#999">地图选择组件加载中…请使用 AmapLocationPicker 组件</p>
      </div>
      <template #footer>
        <el-button @click="showMapPicker = false">关闭</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="showCatDialog" :title="editingCat ? '编辑分类' : '新增分类'" width="400px" destroy-on-close>
      <el-form :model="catForm" label-width="80px">
        ...
        <el-form-item label="图标"><el-input v-model="catForm.icon" /></el-form-item>
...
const locFilters = reactive({ keyword: '', categoryId: '' })
const showLocDialog = ref(false)
const editingLoc = ref<any>(null)
const locForm = reactive({ name: '', categoryId: '', address: '', latitude: 0, longitude: 0, description: '', isEnabled: true })
...
async function loadLocations() {
  ...
  try {
    ...
    locations.value = res.list || res.data?.list || []
    locTotal.value = res.total || res.data?.total || 0
  } catch (e: any) { ElMessage.error(e?.message || '加载失败'); locations.value = [] }
...
function openLocationDialog(row?: any) {
  ...
  if (row) {
    Object.assign(locForm, { name: row.name, categoryId: row.categoryId, address: row.address || '', latitude: row.latitude || 0, longitude: row.longitude || 0, description: row.description || '', isEnabled: row.status === 'PUBLISHED' })
  } else {
    Object.assign(locForm, { name: '', categoryId: '', address: '', latitude: 0, longitude: 0, description: '', isEnabled: true })
  }
...
async function saveLocation() {
  saving.value = true
  try {
    const payload = { ...locForm, status: locForm.isEnabled ? 'PUBLISHED' : 'DRAFT' }
    if (editingLoc.value) {
...
async function saveCategory() {
  ...
  try {
    if (editingCat.value) {
      await request.put(`/admin/punch/categories/${editingCat.value.id}`, catForm)
    } else {
      await request.post('/admin/punch/categories', catForm)
    }
    ElMessage.success('保存成功')
...
async function loadComments() {
  ...
  try {
    ...
    punchComments.value = res.list || res.data?.list || []
    comTotal.value = res.total || res.data?.total || 0
  } catch (e: any) { ElMessage.error(e?.message || '加载失败'); punchComments.value = [] }
...
async function loadStats() {
  ...
  try {
    const res: any = await request.get('/admin/punch/stats/overview')
    stats.value = res || {}
  } catch (e: any) { ElMessage.error(e?.message || '加载失败'); stats.value = {} }
  finally { statsLoading.value = false }
}

function handleTabChange() {
  const loaders: Record<string, () => void> = {
    locations: loadLocations, categories: loadCategories, records: loadRecords,
    comments: loadComments, configs: () => {}, stats: loadStats,
  }
