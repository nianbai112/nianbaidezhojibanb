...
async function ft(){ld.value=true;try{const r=await stickerApi.getCategoryList({page:p.value,pageSize:ps.value});list.value=r.data.data?.list||r.data?.list||[];t.value=r.data.data?.total||r.data?.total||0}catch{}finally{ld.value=false}}
...
async function onSubmit(){const data={...form};if(editingId.value){await stickerApi.updateCategory(editingId.value,data)}else{await stickerApi.createCategory(data)}message.success('保存成功');modalVisible.value=false;ft()}
async function onDelete(r:any){Modal.confirm({title:'确认删除',content:`确定删除「${r.name}」吗？`,onOk:async()=>{await stickerApi.deleteCategory(r.id);message.success('已删除');ft()}})}
ft();
</script>
