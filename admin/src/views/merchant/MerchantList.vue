<template>
  <div class="page-shell">
    <PageHeader :title="pageTitle" :subtitle="pageSubtitle" :icon="isDormShopPage ? 'House' : 'Shop'" />
    <div class="filter-bar">
      ...
      <el-button type="primary" @click="openEdit()">{{ isDormShopPage ? '新增小店' : '新增商家' }}</el-button>
    </div>
    <el-table :data="list" v-loading="loading" border stripe>
      <el-table-column prop="logo" label="Logo" width="70">
        <template #default="{ row }">
          <el-image v-if="row.logo" :src="row.logo" style="width: 40px; height: 40px; border-radius: 6px;" />
          <span v-else>-</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="isDormShopPage" prop="studentVerified" label="学生认证" width="100">
        <template #default="{ row }">
          <el-tag :type="row.studentVerified ? 'success' : 'warning'" size="small">{{ row.studentVerified ? '已认证' : '未确认' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column v-if="isDormShopPage" prop="deliveryMode" label="配送方式" width="110">
        <template #default="{ row }">
          <el-tag type="success" size="small">{{ deliveryModeLabel(row.deliveryMode) }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column v-if="isDormShopPage" prop="deliveryFee" label="配送费" width="90">
        <template #default="{ row }">¥{{ Number(row.deliveryFee || 0).toFixed(2) }}</template>
      </el-table-column>
      <el-table-column v-if="isDormShopPage" prop="businessHours" label="营业时间" width="130">
        <template #default="{ row }">
          <el-tag v-if="row.businessHours" type="info" size="small">{{ businessHoursLabel(row.businessHours) }}</el-tag>
          <el-tag v-else type="danger" size="small">未设置</el-tag>
        </template>
      </el-table-column>
      <el-table-column v-if="isDormShopPage" prop="closedNotice" label="关闭提示" width="160" show-overflow-tooltip>
        <template #default="{ row }">
          <span v-if="row.auditStatus === 'closed'">{{ row.closedNotice || '-' }}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
    ...
    <el-dialog v-model="editVisible" :title="editingId ? (isDormShopPage ? '编辑宿舍小店' : '编辑商家') : (isDormShopPage ? '新增宿舍小店' : '新增商家')" width="760px" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px" :rules="formRules" ref="formRef">
        <el-form-item :label="isDormShopPage ? '店主用户' : '商家用户'" prop="userId">
          <el-select v-model="form.userId" filterable clearable placeholder="选择对应的小程序用户" style="width: 100%">
            <el-option
          ...
          <div class="form-tip">后台必须知道这个小店/商家对应哪个小程序用户，后续订单通知、权限和纠纷追踪都靠它。</div>
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
        ...
        <el-form-item v-if="isDormShopPage && form.status === 'closed'" label="关闭提示" prop="closedNotice">
          <el-input
          ...
          />
          <div class="form-tip">这段文字会在小程序用户点进关闭的小店时弹窗展示。</div>
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item :label="isDormShopPage ? '分类(可选)' : '分类'" prop="categoryId">
              <el-select
                v-model="form.categoryId"
                :placeholder="isDormShopPage ? '可不填，默认全部商品' : '请选择分类'"
                style="width: 100%"
                filterable
                clearable
              ...
              <div v-if="isDormShopPage" class="form-tip">宿舍小店不强制店铺分类；后面商品分类在商品管理里单独维护。</div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="地址" prop="address">
          <div class="address-picker-row">
            <el-input v-model="form.address" placeholder="可手动填写，地图选择后会自动带入" />
            <el-button type="primary" plain :icon="Location" @click="mapVisible = true">地图选点</el-button>
          </div>
          <div class="form-tip">运营不用记经纬度，点“地图选点”搜索宿舍楼或学校位置即可自动回填。</div>
        </el-form-item>
        <el-row v-if="isDormShopPage" :gutter="16">
          <el-col :span="8">
            <el-form-item label="宿舍楼"><el-input v-model="form.dormBuilding" placeholder="如 18" /></el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="房间号"><el-input v-model="form.dormRoom" placeholder="如 304" /></el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="学生认证"><el-switch v-model="form.studentVerified" active-text="已确认" inactive-text="待确认" /></el-form-item>
          </el-col>
        </el-row>
        <el-row v-if="isDormShopPage" :gutter="16">
          <el-col :span="12">
            <el-form-item label="配送方式">
              <el-tag type="success" size="large">店主自送</el-tag>
              <div class="form-tip">宿舍小店不进入骑手池，避免和跑腿/骑手订单混在一起。</div>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="配送费">
              <el-input-number v-model="form.deliveryFee" :min="0" :precision="2" :step="0.5" controls-position="right" style="width: 100%" />
              <div class="form-tip">由店主自送收取，可设为 0 元。</div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="经度"><el-input v-model="form.longitude" placeholder="地图选点后自动生成" /></el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="纬度"><el-input v-model="form.latitude" placeholder="地图选点后自动生成" /></el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="营业时间" prop="businessHoursRange">
          <el-time-picker
            ...
            style="width: 100%"
          />
          <div v-if="isWeeklyBusinessHours(form.businessHours)" class="form-tip">该商家使用每周营业计划，请在商家端维护；本次保存会保留原计划。</div>
          <div v-if="isDormShopPage" class="form-tip">宿舍小店必须选择真实营业时间，小程序只会在这个时间范围内生成配送时间。</div>
        </el-form-item>
    ...
    <el-dialog v-model="detailVisible" :title="detail ? `商家档案 · ${detail.name}` : '商家档案'" width="860px" destroy-on-close>
      <el-tabs v-if="detail" v-model="detailTab" v-loading="detailContextLoading">
        <el-tab-pane label="经营档案" name="overview">
          <el-descriptions :column="2" border>
            ...
            <el-descriptions-item :label="isDormShopPage ? '店主小程序用户' : '商家小程序用户'" :span="2">
              <div v-if="merchantOwner(detail)" class="user-cell">
                <el-avatar :size="36" :src="merchantOwner(detail).avatar">{{ userInitial(merchantOwner(detail)) }}</el-avatar>
                <div class="user-meta"><div class="user-name">{{ merchantOwner(detail).nickname || '未设置昵称' }}</div><div class="user-sub">用户ID：{{ merchantOwner(detail).id }}</div></div>
              </div>
              <el-tag v-else type="danger" size="small">未绑定小程序用户</el-tag>
            </el-descriptions-item>
            <el-descriptions-item label="小程序手机号">{{ merchantOwner(detail)?.phone || '-' }}</el-descriptions-item>
            <el-descriptions-item label="小程序UID">{{ merchantOwner(detail)?.uid ? `UID ${merchantOwner(detail).uid}` : '-' }}</el-descriptions-item>
            ...
            <el-descriptions-item v-if="isDormShopPage" label="配送方式">店主自送</el-descriptions-item>
            <el-descriptions-item v-if="isDormShopPage" label="配送费">¥{{ Number(detail.deliveryFee || 0).toFixed(2) }}</el-descriptions-item>
            <el-descriptions-item label="营业时间">{{ businessHoursLabel(detail.businessHours) }}</el-descriptions-item>
            <el-descriptions-item v-if="detail.status === 'closed'" label="关闭提示" :span="2">{{ detail.closedNotice || '-' }}</el-descriptions-item>
          ...
          <div class="detail-action"><el-button type="primary" plain @click="openMerchantScope('/finance/merchant-settle')">查看该商家结算</el-button></div>
        </el-tab-pane>
        <el-tab-pane :label="`商品（${detailProducts.length}）`" name="products">
          <el-table :data="detailProducts" size="small" max-height="300"><el-table-column prop="name" label="商品" min-width="180" /><el-table-column prop="price" label="售价" width="100"><template #default="{ row }">¥{{ Number(row.price || 0).toFixed(2) }}</template></el-table-column><el-table-column prop="stock" label="库存" width="80" /><el-table-column prop="status" label="状态" width="90"><template #default="{ row }"><el-tag size="small" :type="row.status === 'on_sale' ? 'success' : 'info'">{{ row.status === 'on_sale' ? '上架' : '下架' }}</el-tag></template></el-table-column></el-table>
          <el-empty v-if="!detailProducts.length && !detailContextLoading" description="暂未添加商品" />
          <div class="detail-action"><el-button type="primary" plain @click="openMerchantScope('/merchant/products')">管理该商家商品</el-button></div>
        </el-tab-pane>
        <el-tab-pane :label="`订单（${detailOrders.length}）`" name="orders">
          <el-table :data="detailOrders" size="small" max-height="300"><el-table-column prop="orderNo" label="订单号" min-width="180" /><el-table-column label="实付" width="100"><template #default="{ row }">¥{{ Number(row.payAmount || row.amount || 0).toFixed(2) }}</template></el-table-column><el-table-column label="状态" width="130"><template #default="{ row }">{{ orderStatusLabel(row) }}</template></el-table-column><el-table-column label="下单时间" width="170"><template #default="{ row }">{{ formatDate(row.createdAt) }}</template></el-table-column></el-table>
          <el-empty v-if="!detailOrders.length && !detailContextLoading" description="暂未产生订单" />
          <div class="detail-action"><el-button type="primary" plain @click="openMerchantScope('/merchant/orders')">查看该商家履约订单</el-button></div>
        </el-tab-pane>
        <el-tab-pane :label="`打印设备（${detailPrinters.length}）`" name="printers">
          <el-table :data="detailPrinters" size="small" max-height="300"><el-table-column prop="name" label="设备名称" min-width="180" /><el-table-column prop="brand" label="品牌" width="110"><template #default="{ row }">{{ printerBrandLabel(row.brand) }}</template></el-table-column><el-table-column prop="sn" label="设备编号" min-width="160" /><el-table-column label="状态" width="90"><template #default="{ row }"><el-tag size="small" :type="row.status === 'active' ? 'success' : 'info'">{{ row.status === 'active' ? '启用' : '停用' }}</el-tag></template></el-table-column></el-table>
          <el-empty v-if="!detailPrinters.length && !detailContextLoading" description="暂未配置打印设备" />
          <div class="detail-action"><el-button type="primary" plain @click="openMerchantScope('/merchant/printers')">配置该商家设备</el-button></div>
        </el-tab-pane>
      </el-tabs>
    </el-dialog>
  </div>
...
import { getMerchants, createMerchant, updateMerchant, auditMerchant, updateMerchantStatus, getMerchantDetail, getProducts, getMerchantOrders, getPrinters } from '@/api/merchant'
...
const pageTitle = computed(() => isDormShopPage.value ? '宿舍小店' : '商家管理')
const pageSubtitle = computed(() => isDormShopPage.value ? '管理学生运营的小店、楼栋范围和审核状态' : '管理入驻审核、营业状态与经营资料')
...
const editingId = ref('')
const form = reactive<any>({ userId: '', name: '', contactPerson: '', phone: '', regionId: '', categoryId: '', address: '', dormBuilding: '', dormRoom: '', studentVerified: false, deliveryMode: 'self_delivery', deliveryFee: 0, latitude: '', longitude: '', businessHours: '', businessHoursRange: [], closedNotice: '', description: '', logo: '', cover: '', status: 'pending' })
const formRef = ref<any>(null)
const baseRules = {
  userId: [{ required: true, message: '请选择对应的小程序用户', trigger: 'change' }],
  name: [{ required: true, message: '请输入商家名称', trigger: 'blur' }],
  regionId: [{ required: true, message: '请选择区域', trigger: 'change' }],
}
const validateBusinessHoursRange = (_rule: any, value: string[], callback: (error?: Error) => void) => {
  if (isWeeklyBusinessHours(form.businessHours)) return callback()
  if (!isDormShopPage.value && (!value || value.length === 0)) return callback()
...
const validateClosedNotice = (_rule: any, value: string, callback: (error?: Error) => void) => {
...
const formRules = computed(() => ({
  ...
  ...(isDormShopPage.value
    ? {
        ...
        closedNotice: [
          { validator: validateClosedNotice, trigger: 'blur' },
        ],
      }
...
const orderStatusLabel = (row: any) => ({ PENDING_PAY: '待付款', PAID: '待商家接单', SHIPPED: '配送中', DELIVERED: '已送达', COMPLETED: '已完成', CANCELLED: '已取消', REFUNDING: '退款中', REFUNDED: '已退款' }[row?.status] || row?.status || '-')
const printerBrandLabel = (brand?: string) => ({ feie: '飞鹅云', yly: '易联云', xpyun: '芯烨云', gprinter: '佳博云' }[String(brand || '')] || brand || '-')
const deliveryModeLabel = (value?: string) => value === 'platform_rider' ? '平台配送' : '店主自送'
...
const isWeeklyBusinessHours = (value?: string) => {
  try {
    ...
    return Array.isArray(schedule) && schedule.length === 7 && schedule.every((item) => Array.isArray(item) && item.length === 3)
  } catch {
    return false
  }
}
const businessHoursLabel = (value?: string) => isWeeklyBusinessHours(value) ? '每周计划（商家端维护）' : (value || '-')
const formatBusinessHoursRange = (range: string[] = []) => {
  if (!Array.isArray(range) || range.length !== 2 || !range[0] || !range[1]) return ''
  return `${range[0]}-${range[1]}`
}
const selectedRegion = computed(() => regionList.value.find((item: any) => item.id === form.regionId))
const mapDefaultCenter = computed(() => {
  const formLng = Number(form.longitude)
  const formLat = Number(form.latitude)
  if (Number.isFinite(formLng) && Number.isFinite(formLat) && formLng && formLat) return [formLng, formLat] as [number, number]
  const regionLng = Number(selectedRegion.value?.longitude)
  const regionLat = Number(selectedRegion.value?.latitude)
  if (Number.isFinite(regionLng) && Number.isFinite(regionLat) && regionLng && regionLat) return [regionLng, regionLat] as [number, number]
  return undefined
})
const mapDefaultCity = computed(() => selectedRegion.value?.city || selectedRegion.value?.name || '全国')

const loadData = async () => {
  loading.value = true
  try {
    const res: any = await getMerchants({ page: page.value, pageSize: pageSize.value, businessType: businessType.value, ...filters })
...
const applyRouteFilters = () => {
  ...
  const auditStatus = typeof route.query.auditStatus === 'string' ? route.query.auditStatus : ''
  filters.auditStatus = ['pending', 'approved', 'rejected', 'closed'].includes(auditStatus) ? auditStatus : ''
}
...
const loadOptions = async () => {
  try {
    const [regions, catRes, userRes]: any[] = await Promise.all([
      ...
      request.get('/admin/users', { params: { page: 1, pageSize: 300, status: 'active', userType: 1 } }),
    ])
    regionList.value = regions
    categoryList.value = catRes?.list || catRes?.data?.list || []
    miniUsers.value = Array.isArray(userRes) ? userRes : userRes?.list || userRes?.data?.list || []
  } catch (e: any) {
...
const openEdit = (row?: any) => {
  ...
  if (row) {
    ...
    Object.assign(form, {
      userId: row.userId || row.ownerUserId || row.user?.id || row.ownerUser?.id || '',
      ...
      dormBuilding: row.dormBuilding || '', dormRoom: row.dormRoom || '', studentVerified: !!row.studentVerified,
      deliveryMode: isDormShopPage.value ? 'self_delivery' : (row.deliveryMode || 'platform_rider'),
      deliveryFee: Number(row.deliveryFee || 0),
      latitude: row.latitude || '', longitude: row.longitude || '', businessHours: row.businessHours || '', businessHoursRange: parseBusinessHoursRange(row.businessHours),
    ...
    Object.assign(form, { userId: '', name: '', contactPerson: '', phone: '', regionId: '', categoryId: '', address: '', dormBuilding: '', dormRoom: '', studentVerified: isDormShopPage.value, deliveryMode: isDormShopPage.value ? 'self_delivery' : 'platform_rider', deliveryFee: 0, latitude: '', longitude: '', businessHours: '', businessHoursRange: [], closedNotice: '', description: '', logo: '', cover: '', status: 'pending' })
  }
...
const submitEdit = async () => {
  ...
  try {
    ...
    payload.businessHours = form.businessHoursRange.length ? formatBusinessHoursRange(form.businessHoursRange) : form.businessHours
    payload.closedNotice = form.status === 'closed' ? String(form.closedNotice || '').trim() : null
...
const toggleStatus = async (row: any) => {
  try {
    ...
    if (target === 'closed' && isDormShopPage.value) {
      const { value } = await ElMessageBox.prompt('请输入小程序弹窗提示文案', '关闭宿舍小店', {
        type: 'warning',
        inputType: 'textarea',
        inputValue: row.closedNotice || '',
        inputPlaceholder: '例如：店主临时有事，今晚暂停接单，明天恢复营业。',
        inputValidator: (value) => Boolean(String(value || '').trim()) || '请输入关闭提示',
      })
...
const managePrinters = (row: any) => router.push({ path: '/merchant/printers', query: { merchantId: row.id } })
...
const loadMerchantContext = async (merchantId: string) => {
  ...
  const [products, orders, printers] = await Promise.all([
    getProducts({ page: 1, pageSize: 5, merchantId, businessType: businessType.value }).catch(() => null),
    getMerchantOrders({ page: 1, pageSize: 5, merchantId, businessType: businessType.value }).catch(() => null),
...
watch(() => route.query.auditStatus, () => {
  applyRouteFilters()
  page.value = 1
  loadData()
})

onMounted(() => { applyRouteFilters(); loadData(); loadOptions() })
</script>
...
.address-picker-row { display: flex; width: 100%; gap: 10px; }
.address-picker-row .el-input { flex: 1; }
.address-picker-row .el-button { flex: none; }
...
.detail-action { display: flex; justify-content: flex-end; margin-top: 14px; }
</style>
