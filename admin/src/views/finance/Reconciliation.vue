  ...
  <div class="page-shell">
    ...
    <div class="summary-grid">
      <el-card shadow="never">
        <div class="summary-label">支付收入</div>
        <div class="summary-value">¥{{ Number(summary.income?.total || 0).toFixed(2) }}</div>
        <div class="summary-sub">{{ summary.income?.count || 0 }} 笔</div>
      </el-card>
      <el-card shadow="never">
        <div class="summary-label">退款金额</div>
        <div class="summary-value">¥{{ Number(summary.refund?.total || 0).toFixed(2) }}</div>
        <div class="summary-sub">{{ summary.refund?.count || 0 }} 笔</div>
      </el-card>
      <el-card shadow="never">
        <div class="summary-label">提现金额</div>
        <div class="summary-value">¥{{ Number(summary.withdraw?.total || 0).toFixed(2) }}</div>
        <div class="summary-sub">{{ summary.withdraw?.count || 0 }} 笔</div>
      </el-card>
      <el-card shadow="never">
        <div class="summary-label">净收入</div>
        <div class="summary-value">¥{{ Number(summary.netIncome || 0).toFixed(2) }}</div>
        <div class="summary-sub">支付 - 退款 - 提现</div>
      </el-card>
    </div>
...
.summary-value { color: #101828; font-size: 24px; font-weight: 700; margin-top: 8px; }
