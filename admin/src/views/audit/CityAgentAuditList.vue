...
const columns = [
  ...
  { title: '申请区域', dataIndex: 'region', width: 140 },
...
function onReset() { filter.status = undefined; filter.keyword = ''; onSearch() }
function onTableChange() { fetchList() }

async function onQuickApprove(record: any) {
  auditLoading.value = true
  try {
    await cityAgentApi.auditApplication(record.id, { status: 'approved' })
    message.success('已通过')
    fetchList()
  } catch {
    // handled by interceptor
...
function onOpenReject(record: any) {
