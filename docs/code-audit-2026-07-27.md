...
本报告列出的 22 项问题均已完成代码修复；状态如下：

| 编号 | 状态 | 修复摘要 |
| --- | --- | --- |
| P0-01 | 已修复 | 后端忽略 `backend/storage/`、`.workbuddy/`、`work/`，数据库备份不进入 Git。 |
| P1-01 | 已修复 | 高风险旧 `/api` 兼容控制器统一限制为超级管理员，关闭低权限跨模块、跨区域入口。 |
| P1-02 | 已修复 | 授权、升级、修复和小程序包操作统一增加超级管理员守卫。 |
| P1-03 | 已修复 | 财务总览、补贴、余额流水、提现和区域余额日志统一传入操作者并应用区域范围；提现审核和完成增加资源级校验。 |
| P1-04 | 已修复 | 余额调整改用独立 `finance:balance-adjust` 权限，种子权限、兼容接口和前端按钮同步。 |
| P1-05 | 已修复 | 用户余额扣减改为带余额条件的原子更新，区域余额改为数据库原子增量并记录数据库返回余额。 |
| P1-06 | 已修复 | 权益目标白名单与 `activity_order`、`errand_order`、`shop_order`、`mall_order` 等真实调用统一。 |
| P1-07 | 已修复 | 权益配额采用条件更新，并以 `grantId + targetType + targetId` 唯一键实现业务幂等。 |
| P1-08 | 已修复 | 免费置顶和二手刷新改为权益扣减与业务履约同一数据库事务。 |
| P1-09 | 已修复 | 恢复已发布迁移内容，新增可重放的基线与权益幂等迁移；未再原地追加历史结构。 |
| P1-10 | 已修复 | 补齐帖子管理与操作日志页面缺失的响应式状态、筛选、详情和加载变量。 |
| P1-11 | 已修复 | 密码重置前后端契约统一，服务端执行强度策略、强制改密并立即撤销旧会话。 |
| P2-01 | 已修复 | 更新 3 个过时测试桩；全量后端 86 套、696 项通过。 |
| P2-02 | 已修复 | 管理后台 TypeScript 错误清零。 |
| P2-03 | 已修复 | 升级 Nest、Axios、对象存储、邮件、图像、Vite 等依赖；根工作区与独立后端生产依赖审计均为 0。 |
| P2-04 | 已修复 | 原生 WebSocket 私聊和群聊捕获唯一键冲突并返回重复消息确认。 |
| P2-05 | 已修复 | 契约检查读取真实 HTTP 方法、上传默认 POST、严格模式失败退出，并删除过时权益封装；435/435 匹配。 |
| P2-06 | 已修复 | 前端审查日志、截图、二维码和无引用重复 Logo 已从索引移除并加入忽略规则。 |
| P2-07 | 已修复 | Element Plus 改为按需组件，拆分 Vue 与图标缓存包；主入口由约 1,318 kB 降至约 316 kB。 |
| P2-08 | 已修复 | 删除无效的机器人统一密码界面、默认值和未使用哈希逻辑。 |
| P3-01 | 已修复 | 重复迁移前缀改为唯一序号，并补齐微信模板基线迁移。 |
| P3-02 | 已修复 | 新增只读 `lint:check` 与显式 `lint:fix`，后端和管理后台门禁均可执行通过。 |

额外发现并修复了两份未发布迁移中的 MySQL 反引号语法。使用随机命名的临时 PostgreSQL 空库重放 115 个迁移，全部成功；临时库验证后已删除。
